  ...
  save(): void {
    ...
    if (id) {
      assert(
        ...
        `Entities of type OrderApprovedPartOne must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
...
export class OrderApprovedPartTwo extends Entity {
  constructor(id: string) {
    ...
    this.set("orderbookInclusionDesired", Value.fromBoolean(false));
  }
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type OrderApprovedPartTwo must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
