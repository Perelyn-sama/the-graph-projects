...
async function getAuroraCurveToken(
  App,
  curve,
  address,
  stakingAddress,
...
async function getAuroraStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "basicAuroraVault":
      const basicAuroraVault = new ethcall.Contract(
        tokenAddress,
        HARVEST_VAULT_ABI
      );
...
async function loadAuroraSynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
  ...
  if (!getParameterCaseInsensitive(tokens, rewardTokenAddress)) {
    tokens[rewardTokenAddress] = await getAuroraToken(
      App,
      rewardTokenAddress,
      stakingAddress
    );
...
async function loadAuroraSynthetixPool(
  App,
  tokens,
  prices,
  abi,
  ...
  const info = await loadAuroraSynthetixPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
async function loadAuroraBasisFork(data) {
  ...
  let p1 = await loadAuroraSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SharePool2) {
    let p3 = await loadAuroraSynthetixPool(
      App,
      tokens,
      prices,
      data.PoolABI,
  ...
  let p2 = await loadAuroraSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SeedBanks) {
    let p = await loadMultipleAuroraSynthetixPools(
      App,
      tokens,
      prices,
      data.SeedBanks
...
async function getAuroraPoolInfo(
  app,
  chefContract,
  chefAddress,
  poolIndex,
...
async function loadMultipleAuroraSynthetixPoolsSequential(
  App,
  tokens,
  prices,
  pools
  ...
  for (const p of pools) {
    let res = await loadAuroraSynthetixPool(
      App,
      tokens,
      prices,
      p.abi,
